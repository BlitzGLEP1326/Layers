/****** MAPAPI.H *************** Belenkov O.V.    ***** 11/12/06 ***  // 18/07/07
******* MAPAPI.H *************** Tarakanova I.R.  ***** 12/04/06 ***
******* MAPAPI.H *************** Panteleeva N.A.  ***** 20/07/06 ***
******* MAPAPI.H *************** Shishkova L.A.   ***** 20/04/06 ***
******* MAPAPI.H *************** Kruzhkova E.A.   ***** 07/08/07 ot 11/05/07 ***
******* MAPAPI.H *************** Derevyagina Zh.A.***** 09/09/04 ***
******* MAPAPI.H *************** Shabakov D.A.    ***** 31/01/06 ***
******* MAPAPI.H *************** Dishlenko S.G.   ***** 12/07/07 ***
...
#define MAPACCESSVERSION  20060117

// Версия интерфейса MAPAPI
#define MAPAPIVERSION  0x0905
...
_MAPIMP HMAP _MAPAPI mapCreateTempSite(const char * rscname);
  ...
  // карта открывается новая (Последствия могут быть разными :-)
...
_MAPIMP long int _MAPAPI mapDeleteMap(HMAP hMap);  // 18/06/99
...
_MAPIMP void     _MAPAPI mapGeo42ToGeoEP90Ex(double *Bx, double *Ly, double *H);       // 03/05/05
 ...
 // проекции UTM (эллипсоид Кларка 1880г.) (поддерживается не для всех карт !)
 // Применение :
 // if (mapIsGeoSupported())
 //   {
 //     mapGeo42ToPlaneUTM(hMap,&B,&L);
 //   }
 // hmap  - идентификатор открытых данных
 // Bx,Ly - преобразуемые координаты
 // H     - высота в точке (метры)
...
_MAPIMP long int _MAPAPI mapGetExclusiveSubject(HOBJ info, long int number);
 ...
 // Для созданного объекта (до операции mapCommitObject())
 // возвращает габариты листа на котором расположен объект
 // (но объект может выходить за габариты листа !)
 // info   - идентификатор объекта карты в памяти
...
_MAPIMP long int _MAPAPI mapObjectFrame(HOBJ info,MAPDFRAME * dframe);
 ...
 // force  - признак пересчета габаритов (устанавливается
...
_MAPIMP long int _MAPAPI mapPutText(HOBJ info,const char * text,long int subject);
  ...
  // Установить новое содержание текстовой строки
  // info   - идентификатор объекта карты в памяти
  // text   - адрес новой строки UNICODE
  // subject - номер подобъекта
  // При ошибке возвращает ноль
...
_MAPIMP  double _MAPAPI mapLengthInMap(HOBJ info);             // 23/06/07 // 18/07/07
...
_MAPIMP long _MAPAPI mapCheckFile(const char * name);
...
_MAPIMP  long int _MAPAPI mapAppendPointGeo(HOBJ info, double b, double l,
 ...
 // При ошибке возвращает ноль
 long int _MAPAPI mapInsertPointGeo(HOBJ info, double b, double l,
 ...
 // subject - номер подобъекта (если = 0, обрабатывается объект)
 // При ошибке возвращает ноль
 long int _MAPAPI mapUpdatePointGeo(HOBJ info, double b, double l,
                                    long int number, long int subject);

 long int _MAPAPI mapUpdatePointGeo3D(HOBJ info, double b, double l,
