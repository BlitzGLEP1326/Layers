/****** MTRAPI.H *************** Belenkov O.V.   ****** 31/11/05 ***
******* MTRAPI.H *************** Gorbunov A.V.   ****** 24/11/06 ***
******* MTRAPI.H *************** Kruzhkov A.E.   ****** 26/06/06 ***
******* MTRAPI.H *************** Shabakov D.A.   ****** 08/11/06 ***
******* MTRAPI.H *************** Derevyagina Zh.A.***** 04/04/06 ***
...
_MAPIMP long int _MAPAPI mapGetMtrBlockSize(HMAP hMap,long int number);
  ...
  // hMap   - идентификатор открытой основной векторной карты 
  // number - номер файла в цепочке.
  // Возвращаемое значение 1 соответствует типу "char".
...
_MAPIMP long int _MAPAPI mapSaveMtqPalette(HMAP hMap,               // 28/11/05
  ...
  // Двухинтервальная палитра формируется с использованием трех
...
_MAPIMP long int _MAPAPI mapGetMtqScale(HMAP hMap,long int number);
  ...
  // Запросить данные о проекции матричных данных // 28/11/01
  // hMap    - идентификатор открытой матричной карты
  // number  - номер матрицы в цепочке.
  // projectiondata - адрес структуры, в которой будут размещены
  // данные о проекции
