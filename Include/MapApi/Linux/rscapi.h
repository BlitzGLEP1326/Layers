/****** RSCAPI.H *************** Belenkov   O.V. ****** 09/10/06 ***
******* RSCAPI.H *************** Panteleeva N.A. ****** 16/10/06 ***
...
_MAPIMP  long int _MAPAPI mapGetRscError(HRSC hRsc);          // 03/02/06
...
_MAPIMP  const char * _MAPAPI mapGetRscObjectName(HRSC hRsc,long int incode);
  ...
  // Запросить ключ объекта по внутреннему  коду (порядковому номеру) //14/06/01
  // объекта (с 1)
  // hRsc - идентификатор классификатора карты
  // incode - внутренний код объекта (номер по порядку)
  // При ошибке возвращает пустую строку
