/****** MAPTYPE.H ************** Belenkov O.V.   ****** 14/12/05 ***
******* MAPTYPE.H ************** Kruzhkov A.E.   ****** 31/01/06 ***
******* MAPTYPE.H ************** Gorbunov A.V.   ****** 12/01/06 ***
******* MAPTYPE.H ************** Shishkova L.A.  ****** 05/12/03 ***
******* MAPTYPE.H ************** Panteleeva N.A  ****** 29/07/03 ***
******* MAPTYPE.H ************** Savochkin V.N   ****** 11/12/03 ***
******* MAPTYPE.H ************** Dishlenko S.G.  ****** 07/06/05 ***
******* MAPTYPE.H ************** Shabakov D.A.   ****** 04/07/05 ***
******* MAPTYPE.H ************** Tarakanova I.R. ****** 26/12/05 ***
*                                                                  *
...
typedef struct POLYDATA                       // 20/11/01
{
#ifdef __cplusplus
...
typedef struct POLYDATAEX                     // 30/05/05
{
#ifdef __cplusplus
...
{
  GT_NONEN  =   0,            // Отсутствует
  GT_LINE  =   1,            // Сетка линий
  GT_NODE  =   2,            // Сетка точек
  GT_CROSS =   3,            // Сетка крестов
};
...
typedef union
{
  ...
  unsigned long Long;  // Представление в виде 32-битового поля
...
{           // 11/10/02
   ...
   unsigned long Rezerv     :15;  // резерв
}
  IMAGESIZE;
...
{
  char Name[32]; // Название классификатора (не более 19 символов)
  char Type[32]; // Тип классификатора (не более 25 символов)
  char Code[8];  // Код классификатора (не более 4 символов)
...
#define EXTSEMANTICEND      31004  // КОНЕЦ ДИАПАЗОНА
...
#define SEMSERVICELAST      32811  // КОНЕЦ  ДИАПАЗОНА СПЕЦИАЛЬНЫХ СЕМАНТИК
...
typedef struct BUILDMTW    //  ПАРАМЕТРЫ СОЗДАНИЯ МАТРИЦЫ ВЫСОТ
{
  ...
  long int LimitMatrixFrame;
                      ...
                      // при Method = 1,2,8 (габариты матрицы
  ...
  long int AltitudeMarksNet;
                      // Флаг формирования и использования сетки
                      // треугольников по высотным точкам
                      // (при Method = 1,2,3) :
                      //  0 - сетка треугольников не формируется
                      //  1 - сетка треугольников формируется

  char Reserve[24];   // Должны быть нули
}
  BUILDMTW;
